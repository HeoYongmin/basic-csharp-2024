...
namespace MyExplorer
{
    public partial class FrmMain : Form
    {
        ...
        private void TrvFolder_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            try
            {
                LsvFile.Items.Clear(); // 다른 폴터에 있던 이전파일 정보 삭제
                                       // 현재폴더의 하위폴더 정보 디스플레이
                string[] directories = Directory.GetDirectories(TxtPath.Text);
                foreach (var directoy in directories)
                {
                    DirectoryInfo info = new DirectoryInfo(directoy);
                ...
                foreach (var file in files)
                {
                    ...
                    item.ImageIndex = GetImageIndex(info.Extension); // 확장자의 종료에 따라 이미지번호가 변경
                    LsvFile.Items.Add(item);
                }
            }
            catch (Exception ex)
